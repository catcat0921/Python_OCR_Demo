...
class MyFTP:
    ...
    def upload_files_tree(self, local_path, remote_path):
        """从本地上传目录下多个文件到ftp
           参数:
    ...
    def upload_file_tree(self, local_path, remote_path):
        ...
        try:
            ...
            base_dir, part_path = self.ftp.pwd(), remote_path.split('\\')
            for p in part_path[0:len(part_path)-1]:
                base_dir = base_dir + p + '/'  # 拼接子目录
                try:
                    self.ftp.cwd(base_dir)  # 切换到子目录, 不存在则异常
                except Exception as e:
                    # print('INFO:', e)
                    self.ftp.mkd(p)  # 不存在创建当前子目录
                    self.ftp.cwd(p)
        #self.ftp.cwd(remote_path)
        self.debug_print('切换至远程目录: %s' % self.ftp.pwd())
        local_name_list = [local_path]#os.listdir(local_path)
        self.debug_print('本地文件为: %s' % local_name_list)
        #self.debug_print('判断是否有服务器目录: %s' % os.path.isdir())

        for local_name in local_name_list:
            src = remote_path.split('\\')[-1] #os.path.join(local_path, local_name)
            # print("src路径=========="+src)
            if os.path.isdir(src):
                try:
                    self.ftp.mkd(local_name)
                    ...
                    self.debug_print("目录已存在 %s ,具体错误描述为：%s" % (local_name, err))
                self.debug_print("上传文件： %s" % remote_path)
                # self.debug_print("upload_file_tree()---> 上传src目录： %s" % src)
                self.upload_file_tree(src, local_name)
            else:
                self.debug_print("上传文件： %s" % remote_path)
                self.upload_file( local_name,src)
        # self.ftp.cwd("..")

    def close(self):
        """ 退出ftp
        """
        self.debug_print("FTP退出")
        self.ftp.quit()
        # self.log_file.close()
    ...
    def write_log(self, log_str):
        ...
        format_log_str = "%s ---> %s" % (date_now, log_str)
...
if __name__ == "__main__":
    my_ftp = MyFTP("121.196.55.16",5002)
    #my_ftp.set_pasv(False)
    my_ftp.login("sick", "sick")


    # 下载单个文件
    my_ftp.download_file("0000041.jpg", "0000041.jpg") #FTP服务器目录   本地目录
